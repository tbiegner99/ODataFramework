 ...
 * This is a tree where each unique path is a branch from the current node. When createing an instance, the first path component is null
 * toallow for multiple distinct paths. the root of the tree, then is not the first item in the path, but rather a placeholder.
 * Note that this data structure does not know about subset paths  i.e a/b/c would hide a/b. In this example a would also be a valid path.
 *
 * @author Admin
 *
 */
